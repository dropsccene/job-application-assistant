...
6. 如果岗位涉及 AI/大模型/RAG/Python后端/FastAPI/Agent/LLM 任一方向，**必须**在招呼语中自然地带上这句（不要一字不改地硬塞，融入到话里）：「线上 Demo 免登录试用：https://capstone-ai-kb.onrender.com/docs」。如果岗位是纯测试/运维/前端且跟 AI 完全无关，就跳过这句。
7. **只输出这条招呼语正文**，不要任何解释、前言、引号、或"以下是为您生成的…"这类话，方便直接复制发送。
"""
